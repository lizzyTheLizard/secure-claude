...
export async function loadConfig(): Promise<SecureClaudeConfig> {
  ...
  if (!configExists) {
    ...
    return { tmpFolder: path.join(cwd, DEFAULT_TMP_FOLDER), allowedDomains: [], blockedDomains: [], defaultAllow: false, dnsServers: '1.1.1.1 8.8.8.8', proxy: 'NONE', additionalVolumes: [], deniedPaths: [], mcpPort: DEFAULT_MCP_PORT, enableGitCommands: true, plugins: [], cwd: cwd, projectName: path.basename(cwd) }
  }
